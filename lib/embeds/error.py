...
class GenericErrors:
    ...
    @staticmethod
    async def bad_arg(ctx, error):
        embed = clean_error_embed(ctx)
        embed.description += formatter.shorten(str(error), 100)
        embed.set_footer(
            text=f"For more info do {await formatter.get_prefix(ctx)}help {formatter.get_invoked_name(ctx)}",
    ...
    @staticmethod
    async def bot_missing_permissions(ctx):
        embed = clean_error_embed(ctx)
    ...
    @staticmethod
    async def private_message_only(ctx):
        embed = clean_error_embed(ctx)
...
class EconErrors:
    @staticmethod
    async def daily_already_claimed(ctx, unix_time):
        embed = clean_error_embed(ctx)
        embed.description += (
...
class HelpErrors:
    ...
    async def error_message(ctx, error):
        ...
        embed.set_footer(
            text=f"See '{await formatter.get_prefix(ctx)}help'", icon_url=question_icon
        )
...
class IntroErrors:
    ...
    async def timeout(ctx):
        ...
        embed.set_footer(
            text=f"Please do {await formatter.get_prefix(ctx)}{formatter.get_invoked_name(ctx)} again",
