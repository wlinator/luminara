...
async def cmd(ctx, bet: int):
    ...
    try:
        ...
        if not is_won:
            ...
            await ctx.send(
                embed=EconErrors.out_of_time(ctx), content=ctx.author.mention
            )
            ctx_currency.take_balance(bet)
            ctx_currency.push()
...
def blackjack_show(
    ctx, bet, player_hand, dealer_hand, player_hand_value, dealer_hand_value
):
    current_time = datetime.now(est).strftime("%I:%M %p")
    thumbnail_url = None

    embed = discord.Embed(title="BlackJack", color=discord.Color.dark_orange())
